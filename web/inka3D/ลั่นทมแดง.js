...
{\n\
	...
	float j = i * 0.25;\n\
	vec3 k = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + 0.451) * vec3(0.683544, 0.190354, 0.264107) + vec3(j / (1.0 - i + j)) * 0.5 * 0.35;\n\
	gl_FragColor = vec4(k, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(141009);
					var b2 = d.decompress16(141009);
					var vb = new Float32Array(282018);
					for (var i = 0, j = 0; i < 47003; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 47003];
						vb[j + 2] = b2[i + 94006];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 47003] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 94006] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(187770);
					var ib = new Uint16Array(187770);
					for (var i = 0, j = 0; i < 62590; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 62590];
						ib[j + 2] = b[i + 125180];
					...
					global.scaleOffset3 = new Float32Array([0.0148497, 0.0212743, 0.021175, 0.0, -469.931, 86.1747, -693.852, 0.0]);
					global.scaleOffset2 = new Float32Array([0.015092, 0.0195448, 0.0207271, 0.0, -503.248, 283.08, -688.164, 0.0]);
					global.scaleOffset1 = new Float32Array([0.0122138, 0.0226508, 0.0178544, 0.0, -408.444, 0.0113254, -588.029, 0.0]);
					global.scaleOffset0 = new Float32Array([0.0143429, 0.018698, 0.019774, 0.0, -479.759, 320.114, -661.188, 0.0]);
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 23634, gl.UNSIGNED_SHORT, 0);
					},
					b: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 247452);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 247440);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 38880, gl.UNSIGNED_SHORT, 47268);
					},
					c: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 403116);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 403104);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 107436, gl.UNSIGNED_SHORT, 125028);
					},
					d: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 1000068);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 1000056);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 17820, gl.UNSIGNED_SHORT, 339900);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					var instance = 
					{
						...
						itransforms: new Int32Array(4), ftransforms: new Float32Array(64), 
						fboundingBoxes: new Float32Array(24), 
						sceneSequence: 0,
						deformerSequence: 0,
						renderSequence: 0,
					...
					instance.update = function()
					{
						...
						ftransforms[31] = 1.0;
						itransforms[1] = 1;
						fboundingBoxes[0] = -8.22774;
						fboundingBoxes[1] = 742.223;
						fboundingBoxes[2] = -2.9841;
						fboundingBoxes[3] = 400.222;
						fboundingBoxes[4] = 742.223;
						fboundingBoxes[5] = 585.054;
						fboundingBoxes[6] = -8.71915;
						fboundingBoxes[7] = 923.517;
						fboundingBoxes[8] = -8.98785;
						fboundingBoxes[9] = 494.537;
						fboundingBoxes[10] = 640.446;
						fboundingBoxes[11] = 679.186;
						fboundingBoxes[12] = 16.6587;
						fboundingBoxes[13] = 783.281;
						fboundingBoxes[14] = -3.05175e-5;
						fboundingBoxes[15] = 486.597;
						fboundingBoxes[16] = 697.117;
						fboundingBoxes[17] = 693.862;
						fboundingBoxes[18] = -9.77556;
						fboundingBoxes[19] = 932.804;
						fboundingBoxes[20] = -13.2422;
						fboundingBoxes[21] = 469.99;
						fboundingBoxes[22] = 612.699;
						fboundingBoxes[23] = 647.956;
					};
				...
				objects:
				{
					"file:Arch42_044_obj_1Shape[0]": 0,
					"file:Arch42_044_obj_2Shape[0]": 1,
					"file:Arch42_044_obj_3Shape[0]": 2,
					"file:Arch42_044_obj_4Shape[0]": 3
				}
