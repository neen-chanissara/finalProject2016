var กระบองเพชร = function(engine)
{
	'use strict';
	var s = {
		textures:
...
void main()\n\
{\n\
	vec3 a = _0 * vec3(9.53809e-4, 0.00265849, 9.92072e-4) + vec3(-31.2539, 0.00132924, -32.5077);\n\
	vec3 b = _1;\n\
...
{\n\
	...
	vec3 d = vec3(max(c.x + c.y + c.z, 0.0) * 0.8) * vec3(0.197854, 0.545, 0.14497);\n\
	gl_FragColor = vec4(d, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(78843);
					var b2 = d.decompress16(78843);
					var vb = new Float32Array(157686);
					for (var i = 0, j = 0; i < 26281; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 26281];
						vb[j + 2] = b2[i + 52562];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 26281] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 52562] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(127938);
					var ib = new Uint16Array(127938);
					for (var i = 0, j = 0; i < 42646; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 42646];
						ib[j + 2] = b[i + 85292];
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 127938, gl.UNSIGNED_SHORT, 0);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[0] = 0.129854;
						ftransforms[1] = 0.0;
						ftransforms[2] = 0.0;
						ftransforms[3] = 0.0;
						ftransforms[4] = 0.0;
						ftransforms[5] = 0.0932617;
						ftransforms[6] = 0.0;
						ftransforms[7] = 0.0;
						ftransforms[8] = 0.0;
						ftransforms[9] = 0.0;
						ftransforms[10] = 0.143318;
						ftransforms[11] = 0.0;
						ftransforms[12] = 0.0;
						ftransforms[13] = 0.0;
						ftransforms[14] = 0.0;
						...
						fboundingBoxes[1] = 87.1136;
						fboundingBoxes[2] = 0.0;
						fboundingBoxes[3] = 31.2544;
						fboundingBoxes[4] = 87.1136;
						fboundingBoxes[5] = 32.5082;
					};
