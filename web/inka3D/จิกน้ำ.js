...
{\n\
	...
	vec3 i = texture2D(_c, (_e * vec2(1.0, 8.0) + vec2(-1.0, 0.0)) * vec2(1.0, -1.0) + vec2(0.0, 1.0)).xyz * vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + pow(max(h.x + h.y + h.z, 0.0), 35.5042) * vec3(0.0107423, 0.752132, 0.363897) + vec3(0.764, 0.367, 0.244);\n\
...
';
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.vertexAttribPointer(2, 2, gl.FLOAT, false, 8, 172832);
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer1);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 518508);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 518496);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 16905, gl.UNSIGNED_SHORT, 43212);
					},
					b: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 21600, gl.UNSIGNED_SHORT, 0);
					},
					c: function(instance, shader)
					{
						var global = instance.global;
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer1);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 518412);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 518400);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 6, gl.UNSIGNED_SHORT, 43200);
					}
				},
