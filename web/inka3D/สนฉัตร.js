	...
	var s = {
		...
		scenes:
		{
			"สนฉัตร":
			{
				shaders:
				{
					// mesh, shader 'Tree:Material_004'
					m_Tree_cMaterial__004:
					{
					},
...
{\n\
	...
	vec3 i = vec3(max(f.x + f.y + f.z, 0.0) * 0.8) * vec3(0.0920729, 0.427344, 0.128862) + vec3(pow(max(h.x + h.y + h.z, 0.0), 98.0784) * 0.5) + vec3(0.146196, 0.213672, 0.0649119);\n\
...
';
					...
					{
						var shader = shaders.m_Tree_cMaterial__004;
						var vertexShader = engine.createVertexShader(vsm_Tree_cMaterial__004, "mesh, shader 'Tree:Material_004'");
						var pixelShader = engine.createPixelShader(psm_Tree_cMaterial__004, "mesh, shader 'Tree:Material_004'");
				...
				doneGlobal: function(global)
				{
					...
					gl.deleteProgram(global.shaders.m_Tree_cMaterial__004.program);
					gl.deleteProgram(global.shaders.m_Tree_cMaterial__003.program);
				},

				render: {
					m_Tree_cMaterial__004_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_Tree_cMaterial__004;
						var transfer = instance.shaders.m_Tree_cMaterial__004.transfer;
						var _a = transfer._a;
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 749580);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 749568);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 68400, gl.UNSIGNED_SHORT, 93696);
					},
					b: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 46848, gl.UNSIGNED_SHORT, 0);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.render = function(viewMatrix, projectionMatrix, layerName, renderQueues)
					{
						...
						// mesh, shader 'Tree:Material_004'
						var s_ = instance.shaders.m_Tree_cMaterial__004;
						...
						if (itransforms[0])
						{
							...
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_Tree_cMaterial__004;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_Tree_cMaterial__004_5;
								var data = renderJob.data;
								data[0] = ftransforms[0];
								data[1] = ftransforms[1];
								data[2] = ftransforms[2];
						...
						var uniform = instance.shaders.m_Tree_cMaterial__004.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_Tree_cMaterial__004.renderJobs;
						while (current)
						{
