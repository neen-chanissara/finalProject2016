...
void main()\n\
{\n\
	vec3 a = _0 * vec3(0.00124845, 8.62254e-4, 0.00119827) + vec3(-40.9087, 4.31127e-4, -39.2646);\n\
	vec3 b = _1;\n\
...
{\n\
	...
	vec3 d = (vec3(max(c.x + c.y + c.z, 0.0) * 0.8) + 0.145296) * vec3(0.924051, 0.0818779, 0.827623) + vec3(0.534693, 0.0818779, 0.924051);\n\
	gl_FragColor = vec4(d, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(35916);
					var b2 = d.decompress16(35916);
					var vb = new Float32Array(71832);
					for (var i = 0, j = 0; i < 11972; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 11972];
						vb[j + 2] = b2[i + 23944];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 11972] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 23944] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(24024);
					var ib = new Uint16Array(24024);
					for (var i = 0, j = 0; i < 8008; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 8008];
						ib[j + 2] = b[i + 16016];
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 24024, gl.UNSIGNED_SHORT, 0);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[0] = -0.100402;
						ftransforms[1] = 0.0;
						ftransforms[2] = 0.0;
						ftransforms[3] = 0.0;
						ftransforms[4] = 0.0;
						ftransforms[5] = 0.0668315;
						ftransforms[6] = 0.0;
						ftransforms[7] = 0.0;
						ftransforms[8] = 0.0;
						ftransforms[9] = 0.0;
						ftransforms[10] = -0.0858744;
						ftransforms[11] = 0.0;
						ftransforms[12] = 0.0;
						ftransforms[13] = 0.0;
						ftransforms[14] = 0.0;
						...
						fboundingBoxes[1] = 28.2543;
						fboundingBoxes[2] = 0.0;
						fboundingBoxes[3] = 40.9094;
						fboundingBoxes[4] = 28.2543;
						fboundingBoxes[5] = 39.2652;
					};
