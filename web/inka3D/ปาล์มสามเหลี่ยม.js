var ปาล์มสามเหลี่ยม = function(engine)
{
	'use strict';
	var s = {
		textures:
		...
		scenes:
		{
			"ปาล์มสามเหลี่ยม":
			{
				shaders:
				{
					// mesh, shader 'Palm_Tree:ENV_MP_Iraq_Palm_Tree_02'
					m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02:
					{
					},
					// mesh, shader 'phongE2'
					m_phongE2:
				...
				{
					// mesh, shader 'Palm_Tree:ENV_MP_Iraq_Palm_Tree_02'
					var vsm_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02 =
...
void main()\n\
{\n\
	vec3 a = _0 * vec3(0.00123424, 0.00124842, 0.00168819) + vec3(-39.1006, -43.18, -0.350615);\n\
	vec3 b = _1;\n\
...
';
					var psm_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02 =
'precision highp float;\n\
uniform float f_f;\n\
varying vec3 _d;\n\
void main()\n\
{\n\
	...
	vec3 d = (vec3(max(c.x + c.y + c.z, 0.0) * 0.8) + 0.588) * vec3(0.32, 0.182002, 0.04384);\n\
...
{\n\
	...
	float j = i * 0.49;\n\
	vec3 k = (vec3(max(f.x + f.y + f.z, 0.0) * 0.632479) + 0.299137) * vec3(0.228961, 0.479, 0.072808) + vec3(j / (1.0 - i + j)) * 0.5 * 0.5;\n\
	gl_FragColor = vec4(k, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(3012);
					var b2 = d.decompress16(3012);
					var vb = new Float32Array(6024);
					for (var i = 0, j = 0; i < 1004; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 1004];
						vb[j + 2] = b2[i + 2008];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 1004] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 2008] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(2628);
					var ib = new Uint16Array(2628);
					for (var i = 0, j = 0; i < 876; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 876];
						ib[j + 2] = b[i + 1752];
					...
					{
						var shader = shaders.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02;
						var vertexShader = engine.createVertexShader(vsm_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02, "mesh, shader 'Palm_Tree:ENV_MP_Iraq_Palm_Tree_02'");
						var pixelShader = engine.createPixelShader(psm_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02, "mesh, shader 'Palm_Tree:ENV_MP_Iraq_Palm_Tree_02'");
					...
					{
						var shader = shaders.m_phongE2;
						var vertexShader = engine.createVertexShader(vsm_phongE2, "mesh, shader 'phongE2'");
						var pixelShader = engine.createPixelShader(psm_phongE2, "mesh, shader 'phongE2'");
						var program = shader.program = gl.createProgram();
						gl.attachShader(program, vertexShader);
						gl.attachShader(program, pixelShader);
						gl.bindAttribLocation(program, 1, '_1');
				...
				doneGlobal: function(global)
				{
					...
					gl.deleteProgram(global.shaders.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02.program);
					gl.deleteProgram(global.shaders.m_phongE2.program);
				},

				render: {
					m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02;
						var transfer = instance.shaders.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02.transfer;
						var _a = transfer._a;
					...
					m_phongE2_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_phongE2;
						var transfer = instance.shaders.m_phongE2.transfer;
						var _a = transfer._a;
						var matrix = renderJob.data;
						var flip;
						var _c = instance.fu0_28;
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 525, gl.UNSIGNED_SHORT, 0);
					},
					b: function(instance, shader)
					{
						...
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 0);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 2103, gl.UNSIGNED_SHORT, 1050);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[0] = 0.1126;
						ftransforms[1] = 0.00541228;
						ftransforms[2] = -0.0149464;
						ftransforms[3] = 0.0;
						ftransforms[4] = -0.0170903;
						ftransforms[5] = 0.0136708;
						ftransforms[6] = -0.123801;
						ftransforms[7] = 0.0;
						ftransforms[8] = -0.00417806;
						ftransforms[9] = 0.127351;
						ftransforms[10] = 0.0146395;
						ftransforms[11] = 0.0;
						ftransforms[12] = 0.0;
						ftransforms[13] = 0.0;
						ftransforms[14] = 0.0;
						ftransforms[15] = 1.0;
						itransforms[0] = 1;
						fboundingBoxes[0] = 1.34232;
						fboundingBoxes[1] = -2.27212;
						fboundingBoxes[2] = 54.9671;
						fboundingBoxes[3] = 40.4435;
						fboundingBoxes[4] = 40.9085;
						fboundingBoxes[5] = 55.3186;
					};
					...
					instance.render = function(viewMatrix, projectionMatrix, layerName, renderQueues)
					{
						...
						// mesh, shader 'Palm_Tree:ENV_MP_Iraq_Palm_Tree_02'
						var s_ = instance.shaders.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02;
						...
						s_.renderJobs = null;
						// mesh, shader 'phongE2'
						var s_ = instance.shaders.m_phongE2;
						var uniform = s_.uniform;
						var _b = uniform._b;
						var _a = s_.transfer._a;
						_a[0] = viewMatrix[0];
						...
						if (itransforms[0])
						{
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_phongE2;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_phongE2_5;
							...
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02_5;
								var data = renderJob.data;
								data[0] = ftransforms[0];
								data[1] = ftransforms[1];
								data[2] = ftransforms[2];
						...
						var uniform = instance.shaders.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_Palm__Tree_cENV__MP__Iraq__Palm__Tree__02.renderJobs;
						while (current)
						{
						...
						var shader = global.shaders.m_phongE2;
						gl.useProgram(shader.program);
						var uniform = instance.shaders.m_phongE2.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_phongE2.renderJobs;
						while (current)
						{
							current.render(current);
							current = current.next;
				...
				objects:
				{
					"Palm_Tree:ENV_MP_Iraq_PalmTree_01Shape[0]": 0
				}
