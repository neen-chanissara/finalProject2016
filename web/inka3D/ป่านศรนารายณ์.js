var ป่านศรนารายณ์ = function(engine)
{
	'use strict';
	var s = {
		textures:
		...
		scenes:
		{
			"ป่านศรนารายณ์":
			{
				shaders:
				{
					// mesh, shader 'file:wire_115115116'
					m_file_cwire__115115116:
					{
					}
				},
...
void main()\n\
{\n\
	vec3 a = _0 * vec3(0.0103864, 0.00831869, 0.0107714) + vec3(-145.214, 9.00836, -222.183);\n\
	vec3 b = _1;\n\
...
{\n\
	...
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + 0.451) * vec3(0.197925, 0.473, 0.094127) + vec3(pow(max(h.x + h.y + h.z, 0.0), 34.0) * 0.35);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(70821);
					var b2 = d.decompress16(70821);
					var vb = new Float32Array(141642);
					for (var i = 0, j = 0; i < 23607; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 23607];
						vb[j + 2] = b2[i + 47214];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 23607] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 47214] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(108690);
					var ib = new Uint16Array(108690);
					for (var i = 0, j = 0; i < 36230; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 36230];
						ib[j + 2] = b[i + 72460];
					...
					{
						var shader = shaders.m_file_cwire__115115116;
						var vertexShader = engine.createVertexShader(vsm_file_cwire__115115116, "mesh, shader 'file:wire_115115116'");
						var pixelShader = engine.createPixelShader(psm_file_cwire__115115116, "mesh, shader 'file:wire_115115116'");
				...
				doneGlobal: function(global)
				{
					...
					gl.deleteProgram(global.shaders.m_file_cwire__115115116.program);
				},

				render: {
					m_file_cwire__115115116_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_file_cwire__115115116;
						var transfer = instance.shaders.m_file_cwire__115115116.transfer;
						var _a = transfer._a;
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 108690, gl.UNSIGNED_SHORT, 0);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[0] = 0.00756744;
						ftransforms[1] = 0.0;
						ftransforms[2] = 0.0;
						ftransforms[3] = 0.0;
						ftransforms[4] = 0.0;
						ftransforms[5] = 0.00694897;
						ftransforms[6] = 0.0;
						ftransforms[7] = 0.0;
						ftransforms[8] = 0.0;
						ftransforms[9] = 0.0;
						ftransforms[10] = 0.00987928;
						ftransforms[11] = 0.0;
						ftransforms[12] = 0.0;
						ftransforms[13] = 0.0;
						ftransforms[14] = 0.0;
						ftransforms[15] = 1.0;
						itransforms[0] = 1;
						fboundingBoxes[0] = 195.124;
						fboundingBoxes[1] = 281.591;
						fboundingBoxes[2] = 130.768;
						fboundingBoxes[3] = 340.344;
						fboundingBoxes[4] = 272.586;
						fboundingBoxes[5] = 352.957;
					};
					...
					instance.render = function(viewMatrix, projectionMatrix, layerName, renderQueues)
					{
						...
						// mesh, shader 'file:wire_115115116'
						var s_ = instance.shaders.m_file_cwire__115115116;
						...
						if (itransforms[0])
						{
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_file_cwire__115115116;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_file_cwire__115115116_5;
						...
						var uniform = instance.shaders.m_file_cwire__115115116.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_file_cwire__115115116.renderJobs;
						while (current)
						{
