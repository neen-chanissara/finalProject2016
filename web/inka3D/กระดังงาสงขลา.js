var กระดังงาสงขลา = function(engine)
{
	'use strict';
	var s = {
		textures:
		...
		scenes:
		{
			"กระดังงาสงขลา":
			{
				shaders:
				{
					// mesh, shader 'file1:wire_006135059'
					m_file1_cwire__006135059:
					{
					}
				},
				...
				{
					// mesh, shader 'file1:wire_006135059'
					var vsm_file1_cwire__006135059 =
...
void main()\n\
{\n\
	vec3 a = _0 * vec3(0.007679, 0.00829216, 0.00709644) + vec3(-251.621, 0.00414607, -232.532);\n\
	vec3 b = _1;\n\
...
';
					var psm_file1_cwire__006135059 =
'precision highp float;\n\
uniform float f_f;\n\
varying vec3 _d;\n\
varying vec3 _e;\n\
...
{\n\
	...
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.0275596, 0.0506329, 0.0279303)) * vec3(0.264701, 0.746835, 0.272445) + vec3(pow(max(h.x + h.y + h.z, 0.0), 34.0) * 0.35) + vec3(0.090959, 0.256412, 0.0933547);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(451233);
					var b2 = d.decompress16(451233);
					var vb = new Float32Array(902466);
					for (var i = 0, j = 0; i < 150411; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 150411];
						vb[j + 2] = b2[i + 300822];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 150411] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 300822] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(461538);
					var ib = new Uint16Array(461538);
					for (var i = 0, j = 0; i < 153846; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 153846];
						ib[j + 2] = b[i + 307692];
					...
					{
						var shader = shaders.m_file1_cwire__006135059;
						var vertexShader = engine.createVertexShader(vsm_file1_cwire__006135059, "mesh, shader 'file1:wire_006135059'");
						var pixelShader = engine.createPixelShader(psm_file1_cwire__006135059, "mesh, shader 'file1:wire_006135059'");
				...
				doneGlobal: function(global)
				{
					...
					gl.deleteProgram(global.shaders.m_file1_cwire__006135059.program);
				},

				render: {
					m_file1_cwire__006135059_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_file1_cwire__006135059;
						var transfer = instance.shaders.m_file1_cwire__006135059.transfer;
						var _a = transfer._a;
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 187014, gl.UNSIGNED_SHORT, 0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 1572828);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 1572816);
						gl.drawElements(gl.TRIANGLES, 187014, gl.UNSIGNED_SHORT, 374028);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 3145644);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 3145632);
						gl.drawElements(gl.TRIANGLES, 87510, gl.UNSIGNED_SHORT, 748056);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[0] = 0.0757195;
						ftransforms[1] = 0.0;
						ftransforms[2] = 0.0;
						ftransforms[3] = 0.0;
						ftransforms[4] = 0.0;
						ftransforms[5] = 0.0645146;
						ftransforms[6] = 0.0;
						ftransforms[7] = 0.0;
						ftransforms[8] = 0.0;
						ftransforms[9] = 0.0;
						ftransforms[10] = 0.0561187;
						ftransforms[11] = 0.0;
						ftransforms[12] = 0.0;
						ftransforms[13] = 0.0;
						ftransforms[14] = 0.0;
						...
						fboundingBoxes[1] = 271.717;
						fboundingBoxes[2] = 5.34057e-5;
						fboundingBoxes[3] = 251.625;
						fboundingBoxes[4] = 271.717;
						fboundingBoxes[5] = 232.536;
					};
					...
					instance.render = function(viewMatrix, projectionMatrix, layerName, renderQueues)
					{
						...
						// mesh, shader 'file1:wire_006135059'
						var s_ = instance.shaders.m_file1_cwire__006135059;
						...
						if (itransforms[0])
						{
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_file1_cwire__006135059;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_file1_cwire__006135059_5;
						...
						var uniform = instance.shaders.m_file1_cwire__006135059.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_file1_cwire__006135059.renderJobs;
						while (current)
						{
				...
				objects:
				{
					"file1:iFlowers_02_13_2Shape[0]": 0
				}
