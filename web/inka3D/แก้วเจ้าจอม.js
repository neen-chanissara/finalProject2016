var แก้วเจ้าจอม = function(engine)
{
	'use strict';
	var s = {
		textures:
...
{\n\
	...
	vec3 d = texture2D(_c, (_e * vec2(1.0, 8.0) + vec2(-1.0, 0.0)) * vec2(1.0, -1.0) + vec2(0.0, 1.0)).xyz * vec3(max(c.x + c.y + c.z, 0.0) * 0.8) + vec3(0.272392, 0.522142, 0.632911);\n\
...
';
				...
				draw: {
					...
					b: function(instance, shader)
					{
						var global = instance.global;
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(2, 2, gl.FLOAT, false, 8, 172800);
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer1);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 518412);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 518400);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 16905, gl.UNSIGNED_SHORT, 43200);
					},
					c: function(instance, shader)
					{
						var global = instance.global;
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer1);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 852732);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 852720);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 6, gl.UNSIGNED_SHORT, 77010);
					}
				},
