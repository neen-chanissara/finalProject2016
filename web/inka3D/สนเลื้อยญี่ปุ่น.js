var สนเลื้อยญี่ปุ่น = function(engine)
{
	'use strict';
	var s = {
		textures:
...
{\n\
	...
	vec3 g = c * vec3(0.408248, -0.408248, -0.816497);\n\
	vec3 h = (vec3(0.408248, -0.408248, -0.816497) - (g.x + g.y + g.z) * 2.0 * c) * -(d * inversesqrt(e.x + e.y + e.z));\n\
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.167505, 0.405063, 0.143566)) * vec3(0.154181, 0.253164, 0.144207) + pow(max(h.x + h.y + h.z, 0.0), 2.06) * vec3(0.1176, 0.4745, 0.1176);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';
...
{\n\
	...
	vec3 g = c * vec3(0.408248, -0.408248, -0.816497);\n\
	vec3 h = (vec3(0.408248, -0.408248, -0.816497) - (g.x + g.y + g.z) * 2.0 * c) * -(d * inversesqrt(e.x + e.y + e.z));\n\
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.649577, 0.649577, 0.0)) * vec3(0.0552796, 0.189873, 0.0915507) + pow(max(h.x + h.y + h.z, 0.0), 2.06) * vec3(0.196, 0.3921, 0.196);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 10953, gl.UNSIGNED_SHORT, 313380);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 3139596);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 3139584);
						gl.drawElements(gl.TRIANGLES, 65535, gl.UNSIGNED_SHORT, 335286);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 4712436);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 4712424);
						gl.drawElements(gl.TRIANGLES, 36264, gl.UNSIGNED_SHORT, 466356);
					},
					...
					c: function(instance, shader)
					{
						...
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 1566720);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 7920, gl.UNSIGNED_SHORT, 297540);
					},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[0] = -0.0428224;
						ftransforms[1] = -0.0175909;
						ftransforms[2] = 0.0921195;
						ftransforms[3] = 0.0;
						ftransforms[4] = -0.00381826;
						ftransforms[5] = 0.0511694;
						ftransforms[6] = 0.00799628;
						ftransforms[7] = 0.0;
