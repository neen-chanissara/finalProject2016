...
{\n\
	...
	vec3 g = c * vec3(0.408248, -0.408248, -0.816497);\n\
	vec3 h = (vec3(0.408248, -0.408248, -0.816497) - (g.x + g.y + g.z) * 2.0 * c) * -(d * inversesqrt(e.x + e.y + e.z));\n\
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.167505, 0.405063, 0.143566)) * vec3(0.192276, 0.379746, 0.242797) + pow(max(h.x + h.y + h.z, 0.0), 2.06) * vec3(0.1176, 0.4745, 0.1176);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';
...
';
				...
				draw: {
					...
					b: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 47217, gl.UNSIGNED_SHORT, 131070);
					},
