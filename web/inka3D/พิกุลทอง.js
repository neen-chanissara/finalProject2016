...
void main()\n\
{\n\
	vec3 a = _0 * vec3(0.00905195, 0.0136951, 0.0110586) + vec3(-316.577, 0.00684758, -236.085);\n\
	vec3 b = _1;\n\
...
{\n\
	...
	vec3 d = vec3(max(c.x + c.y + c.z, 0.0) * 0.8) * 0.886076 + vec3(0.178235, 0.392405, 0.149014);\n\
	gl_FragColor = vec4(d, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(158391);
					var b2 = d.decompress16(158391);
					var vb = new Float32Array(316782);
					for (var i = 0, j = 0; i < 52797; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 52797];
						vb[j + 2] = b2[i + 105594];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 52797] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 105594] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(63408);
					var ib = new Uint16Array(63408);
					for (var i = 0, j = 0; i < 21136; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 21136];
						ib[j + 2] = b[i + 42272];
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 63408, gl.UNSIGNED_SHORT, 0);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[0] = 0.0600898;
						ftransforms[1] = 0.0;
						ftransforms[2] = 0.0;
						ftransforms[3] = 0.0;
						ftransforms[4] = 0.0;
						ftransforms[5] = 0.0398498;
						ftransforms[6] = 0.0;
						ftransforms[7] = 0.0;
						ftransforms[8] = 0.0;
						ftransforms[9] = 0.0;
						ftransforms[10] = 0.0611666;
						ftransforms[11] = 0.0;
						ftransforms[12] = 0.0;
						ftransforms[13] = 0.0;
						ftransforms[14] = 0.0;
						ftransforms[15] = 1.0;
						itransforms[0] = 1;
						fboundingBoxes[0] = -19.9671;
						fboundingBoxes[1] = 448.763;
						fboundingBoxes[2] = 126.278;
						fboundingBoxes[3] = 296.614;
						fboundingBoxes[4] = 448.763;
						fboundingBoxes[5] = 362.369;
					};
