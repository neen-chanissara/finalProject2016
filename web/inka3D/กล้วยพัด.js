...
{\n\
	...
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + 0.451) * vec3(0.286909, 0.603, 0.167634) + vec3(pow(max(h.x + h.y + h.z, 0.0), 34.0) * 0.35);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(195000);
					var b2 = d.decompress16(195000);
					var vb = new Float32Array(390000);
					for (var i = 0, j = 0; i < 65000; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 65000];
						vb[j + 2] = b2[i + 130000];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 65000] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 130000] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(201756);
					var ib = new Uint16Array(201756);
					for (var i = 0, j = 0; i < 67252; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 67252];
						ib[j + 2] = b[i + 134504];
					...
					global.scaleOffset2 = new Float32Array([0.116014, 0.110258, 0.00695344, 0.0, -3806.61, 0.0551292, -325.042, 0.0]);
					global.scaleOffset1 = new Float32Array([0.0652705, 0.056388, 0.0167006, 0.0, -3765.49, 3519.66, -556.567, 0.0]);
					global.scaleOffset0 = new Float32Array([0.0557725, 0.0519662, 0.0165643, 0.0, 151.584, 3426.18, -528.977, 0.0]);
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 84108, gl.UNSIGNED_SHORT, 0);
					},
					b: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 674604);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 674592);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 88692, gl.UNSIGNED_SHORT, 168216);
					},
					c: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 1382412);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 1382400);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 28956, gl.UNSIGNED_SHORT, 345600);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[31] = 1.0;
						itransforms[1] = 1;
						ftransforms[32] = 0.0162482;
						ftransforms[33] = 0.0;
						ftransforms[34] = 0.0;
						ftransforms[35] = 0.0;
						ftransforms[36] = 0.0;
						ftransforms[37] = 0.00903913;
						ftransforms[38] = 0.0;
						ftransforms[39] = 0.0;
						ftransforms[40] = 0.0;
						ftransforms[41] = 0.0;
						ftransforms[42] = -0.0436504;
						ftransforms[43] = 0.0;
						ftransforms[44] = 0.0;
						ftransforms[45] = 0.0;
						ftransforms[46] = 0.0;
						...
						ftransforms[15] = 1.0;
						itransforms[0] = 1;
						fboundingBoxes[0] = 1979.11;
						fboundingBoxes[1] = 5128.98;
						fboundingBoxes[2] = 13.7947;
						fboundingBoxes[3] = 1827.55;
						fboundingBoxes[4] = 1702.83;
						fboundingBoxes[5] = 542.781;
						fboundingBoxes[6] = -1626.73;
						fboundingBoxes[7] = 5367.36;
						fboundingBoxes[8] = -9.32791;
						fboundingBoxes[9] = 2138.78;
						fboundingBoxes[10] = 1847.72;
						fboundingBoxes[11] = 547.247;
						fboundingBoxes[12] = -5.10192;
						fboundingBoxes[13] = 3612.94;
						fboundingBoxes[14] = -97.195;
						fboundingBoxes[15] = 3801.56;
						fboundingBoxes[16] = 3612.94;
						fboundingBoxes[17] = 227.85;
					};
				...
				objects:
				{
					"file1:archmodels85_016_01Shape[0]": 0,
					"file1:archmodels85_016_02Shape[0]": 1,
					"file1:archmodels85_016_03Shape[0]": 2
				}
