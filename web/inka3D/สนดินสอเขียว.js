var สนดินสอเขียว = function(engine)
{
	'use strict';
	var s = {
		textures:
...
{\n\
	...
	vec3 g = c * vec3(0.408248, -0.408248, -0.816497);\n\
	vec3 h = (vec3(0.408248, -0.408248, -0.816497) - (g.x + g.y + g.z) * 2.0 * c) * -(d * inversesqrt(e.x + e.y + e.z));\n\
	vec3 i = vec3(max(f.x + f.y + f.z, 0.0) * 0.8) * vec3(0.27409, 0.56962, 0.0721038) + vec3(pow(max(h.x + h.y + h.z, 0.0), 98.0784) * 0.5) + vec3(0.146196, 0.213672, 0.0649119);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';
