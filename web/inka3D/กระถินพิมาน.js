var กระถินพิมาน = function(engine)
{
	'use strict';
	var s = {
		textures:
		...
		scenes:
		{
			"กระถินพิมาน":
			{
				shaders:
				{
					...
					// mesh, shader 'file3:wire_006135059'
					m_file3_cwire__006135059:
					{
					}
				},
...
';
					...
					// mesh, shader 'file3:wire_006135059'
					var vsm_file3_cwire__006135059 =
'precision highp float;\n\
uniform vec4 _b[4];\n\
uniform vec4 _c[7];\n\
attribute vec3 _1;\n\
...
void main()\n\
{\n\
	vec3 a = _0 * vec3(0.00357831, 0.00713078, 0.00351086) + vec3(-117.252, 0.00356539, -115.042);\n\
	vec3 b = _1;\n\
	vec3 c = _c[0].xyz * a.x + _c[1].xyz * a.y + _c[2].xyz * a.z + _c[3].xyz;\n\
	gl_Position = _b[0] * c.x + _b[1] * c.y + _b[2] * c.z + _b[3];\n\
	_d = _c[4].xyz * b.x + _c[5].xyz * b.y + _c[6].xyz * b.z;\n\
...
';
					var psm_file3_cwire__006135059 =
'precision highp float;\n\
uniform float f_f;\n\
varying vec3 _d;\n\
varying vec3 _e;\n\
...
{\n\
	...
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.0235, 0.5294, 0.2275)) * vec3(0.0235, 0.5294, 0.2275) + vec3(pow(max(h.x + h.y + h.z, 0.0), 34.0) * 0.35);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(230964);
					var b2 = d.decompress16(230964);
					var vb = new Float32Array(461928);
					for (var i = 0, j = 0; i < 76988; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 76988];
						vb[j + 2] = b2[i + 153976];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 76988] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 153976] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(275418);
					var ib = new Uint16Array(275418);
					for (var i = 0, j = 0; i < 91806; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 91806];
						ib[j + 2] = b[i + 183612];
					...
					{
						var shader = shaders.m_file3_cwire__006135059;
						var vertexShader = engine.createVertexShader(vsm_file3_cwire__006135059, "mesh, shader 'file3:wire_006135059'");
						var pixelShader = engine.createPixelShader(psm_file3_cwire__006135059, "mesh, shader 'file3:wire_006135059'");
						var program = shader.program = gl.createProgram();
						gl.attachShader(program, vertexShader);
						gl.attachShader(program, pixelShader);
						gl.bindAttribLocation(program, 1, '_1');
				...
				doneGlobal: function(global)
				{
					...
					gl.deleteProgram(global.shaders.m_file3_cwire__006135059.program);
				},
				...
				render: {
					...
					m_file3_cwire__006135059_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_file3_cwire__006135059;
						var transfer = instance.shaders.m_file3_cwire__006135059.transfer;
						var _a = transfer._a;
						var matrix = renderJob.data;
						var flip;
						var _c = instance.fu0_28;
				...
				draw: {
					...
					b: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 119934, gl.UNSIGNED_SHORT, 310968);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[15] = 1.0;
						itransforms[0] = 1;
						ftransforms[16] = 0.0352886;
						ftransforms[17] = 0.0;
						ftransforms[18] = 0.0;
						ftransforms[19] = 0.0;
						ftransforms[20] = 0.0;
						ftransforms[21] = 0.0390659;
						ftransforms[22] = 0.0;
						ftransforms[23] = 0.0;
						ftransforms[24] = 0.0;
						ftransforms[25] = 0.0;
						ftransforms[26] = 0.0509281;
						ftransforms[27] = 0.0;
						ftransforms[28] = 0.0;
						ftransforms[29] = 0.0;
						ftransforms[30] = 0.0;
						...
						fboundingBoxes[6] = 5.34057e-5;
						fboundingBoxes[7] = 233.661;
						fboundingBoxes[8] = 4.9591e-5;
						fboundingBoxes[9] = 117.254;
						fboundingBoxes[10] = 233.661;
						fboundingBoxes[11] = 115.043;
					};
					...
					instance.render = function(viewMatrix, projectionMatrix, layerName, renderQueues)
					{
						...
						s_.renderJobs = null;
						// mesh, shader 'file3:wire_006135059'
						var s_ = instance.shaders.m_file3_cwire__006135059;
						var uniform = s_.uniform;
						var _b = uniform._b;
						var _a = s_.transfer._a;
						_a[0] = viewMatrix[0];
						...
						if (itransforms[1])
						{
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_file3_cwire__006135059;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_file3_cwire__006135059_5;
								var data = renderJob.data;
						...
						var shader = global.shaders.m_file3_cwire__006135059;
						gl.useProgram(shader.program);
						var uniform = instance.shaders.m_file3_cwire__006135059.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_file3_cwire__006135059.renderJobs;
						while (current)
						{
							current.render(current);
							current = current.next;
				...
				objects:
				{
					...
					"file3:iFlowers_02_14_6Shape[0]": 1
				}
