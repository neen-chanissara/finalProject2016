...
{\n\
	...
	vec3 b = a * a;\n\
	vec3 c = a * inversesqrt(b.x + b.y + b.z) * f_f * vec3(-0.408248, 0.408248, 0.816497);\n\
	vec3 d = texture2D(_c, (_e * vec2(1.0, 8.0) + vec2(-1.0, 0.0)) * vec2(1.0, -1.0) + vec2(0.0, 1.0)).xyz * vec3(max(c.x + c.y + c.z, 0.0) * 0.8) + vec3(0.721519, 0.456657, 0.693741);\n\
	gl_FragColor = vec4(d, 1.0);\n\
}\n\
';
