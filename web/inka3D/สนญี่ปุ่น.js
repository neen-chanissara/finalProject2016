	...
	var s = {
		...
		scenes:
		{
			"สนญี่ปุ่น":
			{
				shaders:
				{
					// mesh, shader 'plants3:tree_lik_Intrnd1'
					m_plants3_ctree__lik__Intrnd1:
					{
					},
					// mesh, shader 'plants3:tree_lik_Petiole1'
					m_plants3_ctree__lik__Petiole1:
					{
					},
					// mesh, shader 'plants3:tree_lik_Leaf1'
					m_plants3_ctree__lik__Leaf1:
					{
					},
					// mesh, shader 'plants3:tree_lik_Mrstm1'
					m_plants3_ctree__lik__Mrstm1:
					{
					},
				...
				{
					// mesh, shader 'plants3:tree_lik_Intrnd1'
					var vsm_plants3_ctree__lik__Intrnd1 =
...
';
					var psm_plants3_ctree__lik__Intrnd1 =
'precision highp float;\n\
uniform float f_f;\n\
varying vec3 _d;\n\
varying vec3 _e;\n\
...
{\n\
	...
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.2901, 0.2901, 0.1411)) * vec3(0.2901, 0.2901, 0.1411) + pow(max(h.x + h.y + h.z, 0.0), 2.06) * vec3(0.2901, 0.2901, 0.1411);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';

					// mesh, shader 'plants3:tree_lik_Petiole1'
					var vsm_plants3_ctree__lik__Petiole1 =
'precision highp float;\n\
uniform vec4 _b[4];\n\
uniform vec4 _c[7];\n\
attribute vec3 _1;\n\
...
';
					var psm_plants3_ctree__lik__Petiole1 =
'precision highp float;\n\
uniform float f_f;\n\
varying vec3 _d;\n\
varying vec3 _e;\n\
...
';
					...
					// mesh, shader 'plants3:tree_lik_Leaf1'
					var vsm_plants3_ctree__lik__Leaf1 =
'precision highp float;\n\
uniform vec4 _b[4];\n\
uniform vec4 _c[7];\n\
attribute vec3 _1;\n\
...
';
					var psm_plants3_ctree__lik__Leaf1 =
'precision highp float;\n\
uniform float f_f;\n\
varying vec3 _d;\n\
varying vec3 _e;\n\
...
{\n\
	...
	vec3 g = c * vec3(0.408248, -0.408248, -0.816497);\n\
	vec3 h = (vec3(0.408248, -0.408248, -0.816497) - (g.x + g.y + g.z) * 2.0 * c) * -(d * inversesqrt(e.x + e.y + e.z));\n\
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.1176, 0.4745, 0.1176)) * vec3(0.1176, 0.4745, 0.1176) + pow(max(h.x + h.y + h.z, 0.0), 2.06) * vec3(0.1176, 0.4745, 0.1176);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';

					// mesh, shader 'plants3:tree_lik_Mrstm1'
					var vsm_plants3_ctree__lik__Mrstm1 =
'precision highp float;\n\
uniform vec4 _b[4];\n\
uniform vec4 _c[7];\n\
attribute vec3 _1;\n\
...
';
					var psm_plants3_ctree__lik__Mrstm1 =
'precision highp float;\n\
uniform float f_f;\n\
varying vec3 _d;\n\
varying vec3 _e;\n\
...
{\n\
	...
	vec3 g = c * vec3(0.408248, -0.408248, -0.816497);\n\
	vec3 h = (vec3(0.408248, -0.408248, -0.816497) - (g.x + g.y + g.z) * 2.0 * c) * -(d * inversesqrt(e.x + e.y + e.z));\n\
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.196, 0.3921, 0.196)) * vec3(0.192276, 0.379746, 0.242797) + pow(max(h.x + h.y + h.z, 0.0), 2.06) * vec3(0.196, 0.3921, 0.196);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';
...
';
					...
					var b1 = d.decompress8(1442916);
					var b2 = d.decompress16(1442916);
					var vb = new Float32Array(2885832);
					for (var i = 0, j = 0; i < 480972; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 480972];
						vb[j + 2] = b2[i + 961944];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 480972] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 961944] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(555204);
					var ib = new Uint16Array(555204);
					for (var i = 0, j = 0; i < 185068; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 185068];
						ib[j + 2] = b[i + 370136];
					...
					{
						var shader = shaders.m_plants3_ctree__lik__Intrnd1;
						var vertexShader = engine.createVertexShader(vsm_plants3_ctree__lik__Intrnd1, "mesh, shader 'plants3:tree_lik_Intrnd1'");
						var pixelShader = engine.createPixelShader(psm_plants3_ctree__lik__Intrnd1, "mesh, shader 'plants3:tree_lik_Intrnd1'");
					...
					{
						var shader = shaders.m_plants3_ctree__lik__Petiole1;
						var vertexShader = engine.createVertexShader(vsm_plants3_ctree__lik__Petiole1, "mesh, shader 'plants3:tree_lik_Petiole1'");
						var pixelShader = engine.createPixelShader(psm_plants3_ctree__lik__Petiole1, "mesh, shader 'plants3:tree_lik_Petiole1'");
						var program = shader.program = gl.createProgram();
						gl.attachShader(program, vertexShader);
						gl.attachShader(program, pixelShader);
						gl.bindAttribLocation(program, 1, '_1');
					...
					{
						var shader = shaders.m_plants3_ctree__lik__Leaf1;
						var vertexShader = engine.createVertexShader(vsm_plants3_ctree__lik__Leaf1, "mesh, shader 'plants3:tree_lik_Leaf1'");
						var pixelShader = engine.createPixelShader(psm_plants3_ctree__lik__Leaf1, "mesh, shader 'plants3:tree_lik_Leaf1'");
						var program = shader.program = gl.createProgram();
						gl.attachShader(program, vertexShader);
						gl.attachShader(program, pixelShader);
						gl.bindAttribLocation(program, 1, '_1');
					...
					{
						var shader = shaders.m_plants3_ctree__lik__Mrstm1;
						var vertexShader = engine.createVertexShader(vsm_plants3_ctree__lik__Mrstm1, "mesh, shader 'plants3:tree_lik_Mrstm1'");
						var pixelShader = engine.createPixelShader(psm_plants3_ctree__lik__Mrstm1, "mesh, shader 'plants3:tree_lik_Mrstm1'");
						var program = shader.program = gl.createProgram();
						gl.attachShader(program, vertexShader);
						gl.attachShader(program, pixelShader);
						gl.bindAttribLocation(program, 1, '_1');
				...
				doneGlobal: function(global)
				{
					...
					gl.deleteProgram(global.shaders.m_plants3_ctree__lik__Intrnd1.program);
					gl.deleteProgram(global.shaders.m_plants3_ctree__lik__Petiole1.program);
					gl.deleteProgram(global.shaders.m_plants3_ctree__lik__Leaf1.program);
					gl.deleteProgram(global.shaders.m_plants3_ctree__lik__Mrstm1.program);
				...
				render: {
					m_plants3_ctree__lik__Intrnd1_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_plants3_ctree__lik__Intrnd1;
						var transfer = instance.shaders.m_plants3_ctree__lik__Intrnd1.transfer;
						var _a = transfer._a;
					...
					m_plants3_ctree__lik__Petiole1_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_plants3_ctree__lik__Petiole1;
						var transfer = instance.shaders.m_plants3_ctree__lik__Petiole1.transfer;
						var _a = transfer._a;
						var matrix = renderJob.data;
						var flip;
						var _c = instance.fu0_28;
					...
					m_plants3_ctree__lik__Leaf1_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_plants3_ctree__lik__Leaf1;
						var transfer = instance.shaders.m_plants3_ctree__lik__Leaf1.transfer;
						var _a = transfer._a;
						var matrix = renderJob.data;
						var flip;
						var _c = instance.fu0_28;
					...
					m_plants3_ctree__lik__Mrstm1_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_plants3_ctree__lik__Mrstm1;
						var transfer = instance.shaders.m_plants3_ctree__lik__Mrstm1.transfer;
						var _a = transfer._a;
						var matrix = renderJob.data;
						var flip;
						var _c = instance.fu0_28;
				...
				draw: {
					...
					b: function(instance, shader)
					{
						var global = instance.global;
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 7338396);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 7338384);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 2793, gl.UNSIGNED_SHORT, 884904);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 8911260);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 8911248);
						gl.drawElements(gl.TRIANGLES, 65535, gl.UNSIGNED_SHORT, 890490);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 10484100);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 10484088);
						gl.drawElements(gl.TRIANGLES, 44424, gl.UNSIGNED_SHORT, 1021560);
					},
					c: function(instance, shader)
					{
						...
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 7338384);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 8160, gl.UNSIGNED_SHORT, 868584);
					},
					d: function(instance, shader)
					{
						var global = instance.global;
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 5771676);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 5771664);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 65535, gl.UNSIGNED_SHORT, 555204);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 7344516);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 7344504);
						gl.drawElements(gl.TRIANGLES, 83235, gl.UNSIGNED_SHORT, 686274);
					},
					e: function(instance, shader)
					{
						...
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 7338384);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 7920, gl.UNSIGNED_SHORT, 852744);
					},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[0] = 0.147749;
						ftransforms[1] = 0.0;
						ftransforms[2] = 0.0;
						ftransforms[3] = 0.0;
						ftransforms[4] = 0.0;
						ftransforms[5] = 0.122805;
						ftransforms[6] = 0.0;
						ftransforms[7] = 0.0;
						ftransforms[8] = 0.0;
						ftransforms[9] = 0.0;
						ftransforms[10] = 0.197762;
						ftransforms[11] = 0.0;
						ftransforms[12] = 0.0;
						ftransforms[13] = 0.0;
						ftransforms[14] = 0.0;
						ftransforms[15] = 1.0;
						itransforms[0] = 1;
						ftransforms[16] = -0.0568552;
						ftransforms[17] = -0.0233555;
						ftransforms[18] = 0.122306;
						ftransforms[19] = 0.0;
						ftransforms[20] = -0.00906015;
						ftransforms[21] = 0.121416;
						ftransforms[22] = 0.0189738;
						ftransforms[23] = 0.0;
						ftransforms[24] = -0.0946676;
						ftransforms[25] = -1.81697e-4;
						ftransforms[26] = -0.0440416;
						ftransforms[27] = 0.0;
						ftransforms[28] = 0.0;
						ftransforms[29] = 0.0;
						ftransforms[30] = 0.0;
						...
						fboundingBoxes[6] = -178.497;
						fboundingBoxes[7] = 467.996;
						fboundingBoxes[8] = 9.83791;
						fboundingBoxes[9] = 360.62;
						fboundingBoxes[10] = 468.274;
						fboundingBoxes[11] = 274.657;
					};
					...
					instance.render = function(viewMatrix, projectionMatrix, layerName, renderQueues)
					{
						...
						// mesh, shader 'plants3:tree_lik_Intrnd1'
						var s_ = instance.shaders.m_plants3_ctree__lik__Intrnd1;
						...
						s_.renderJobs = null;
						// mesh, shader 'plants3:tree_lik_Petiole1'
						var s_ = instance.shaders.m_plants3_ctree__lik__Petiole1;
						var uniform = s_.uniform;
						var _b = uniform._b;
						var _a = s_.transfer._a;
						_a[0] = viewMatrix[0];
						...
						s_.renderJobs = null;
						// mesh, shader 'plants3:tree_lik_Leaf1'
						var s_ = instance.shaders.m_plants3_ctree__lik__Leaf1;
						var uniform = s_.uniform;
						var _b = uniform._b;
						var _a = s_.transfer._a;
						_a[0] = viewMatrix[0];
						...
						s_.renderJobs = null;
						// mesh, shader 'plants3:tree_lik_Mrstm1'
						var s_ = instance.shaders.m_plants3_ctree__lik__Mrstm1;
						var uniform = s_.uniform;
						var _b = uniform._b;
						var _a = s_.transfer._a;
						_a[0] = viewMatrix[0];
						...
						if (itransforms[0])
						{
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_plants3_ctree__lik__Intrnd1;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_plants3_ctree__lik__Intrnd1_5;
							...
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_plants3_ctree__lik__Petiole1;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_plants3_ctree__lik__Petiole1_5;
								var data = renderJob.data;
								data[0] = ftransforms[0];
								data[1] = ftransforms[1];
								data[2] = ftransforms[2];
							...
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_plants3_ctree__lik__Leaf1;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_plants3_ctree__lik__Leaf1_5;
								var data = renderJob.data;
								data[0] = ftransforms[0];
								data[1] = ftransforms[1];
								data[2] = ftransforms[2];
							...
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_plants3_ctree__lik__Mrstm1;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_plants3_ctree__lik__Mrstm1_5;
								var data = renderJob.data;
								data[0] = ftransforms[0];
								data[1] = ftransforms[1];
								data[2] = ftransforms[2];
						...
						var uniform = instance.shaders.m_plants3_ctree__lik__Intrnd1.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_plants3_ctree__lik__Intrnd1.renderJobs;
						while (current)
						{
						...
						var shader = global.shaders.m_plants3_ctree__lik__Petiole1;
						gl.useProgram(shader.program);
						var uniform = instance.shaders.m_plants3_ctree__lik__Petiole1.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_plants3_ctree__lik__Petiole1.renderJobs;
						while (current)
						{
							current.render(current);
							current = current.next;
						...
						var shader = global.shaders.m_plants3_ctree__lik__Leaf1;
						gl.useProgram(shader.program);
						var uniform = instance.shaders.m_plants3_ctree__lik__Leaf1.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_plants3_ctree__lik__Leaf1.renderJobs;
						while (current)
						{
							current.render(current);
							current = current.next;
						...
						var shader = global.shaders.m_plants3_ctree__lik__Mrstm1;
						gl.useProgram(shader.program);
						var uniform = instance.shaders.m_plants3_ctree__lik__Mrstm1.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_plants3_ctree__lik__Mrstm1.renderJobs;
						while (current)
						{
							current.render(current);
							current = current.next;
				...
				objects:
				{
					"plants3:tree_likShape[0]": 0,
					"plants5:tree_likShape[0]": 1
				}
