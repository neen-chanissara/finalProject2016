...
void main()\n\
{\n\
	vec3 a = _0 * vec3(0.00176842, 0.00223014, 0.00176647) + vec3(-57.3431, -0.552485, -58.8154);\n\
	vec3 b = _1;\n\
...
{\n\
	...
	vec3 d = (vec3(max(c.x + c.y + c.z, 0.0) * 0.8) + 0.6) * 0.6;\n\
...
{\n\
	...
	vec3 i = (vec3(max(f.x + f.y + f.z, 0.0) * 0.8) + vec3(0.3, 0.3, 0.2)) * vec3(0.3, 0.3, 0.2) + vec3(pow(max(h.x + h.y + h.z, 0.0), 12.0) * 0.3);\n\
	gl_FragColor = vec4(i, 1.0);\n\
}\n\
';

					var d = new engine.Decompressor(new Uint8Array(data, 0));
					var b1 = d.decompress8(271428);
					var b2 = d.decompress16(271428);
					var vb = new Float32Array(542856);
					for (var i = 0, j = 0; i < 90476; ++i, j += 6)
					{
						vb[j + 0] = b2[i + 0];
						vb[j + 1] = b2[i + 90476];
						vb[j + 2] = b2[i + 180952];
						vb[j + 3] = (b1[i + 0] << 24) * 4.65661e-10;
						vb[j + 4] = (b1[i + 90476] << 24) * 4.65661e-10;
						vb[j + 5] = (b1[i + 180952] << 24) * 4.65661e-10;
					}
					gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0 = gl.createBuffer());
					gl.bufferData(gl.ARRAY_BUFFER, vb, gl.STATIC_DRAW);

					var b = d.decompress16(312957);
					var ib = new Uint16Array(312957);
					for (var i = 0, j = 0; i < 104319; ++i, j += 3)
					{
						ib[j] = b[i];
						ib[j + 1] = b[i + 104319];
						ib[j + 2] = b[i + 208638];
					...
					global.scaleOffset1 = new Float32Array([0.00176842, 0.00223014, 0.00176647, 0.0, -57.3431, -0.552485, -58.8154, 0.0]);
					global.scaleOffset0 = new Float32Array([0.00179017, 0.00162847, 0.00175986, 0.0, -58.6528, 35.5459, -58.5846, 0.0]);

					var shaders = global.shaders;
					{
						var shader = shaders.m_Tree_cStem1;
						var vertexShader = engine.createVertexShader(vsm_Tree_cStem1, "mesh, shader 'Tree:Stem1'");
						var pixelShader = engine.createPixelShader(psm_Tree_cStem1, "mesh, shader 'Tree:Stem1'");
					...
					{
						var shader = shaders.m_Tree_cL3;
						var vertexShader = engine.createVertexShader(vsm_Tree_cL3, "mesh, shader 'Tree:L3'");
						var pixelShader = engine.createPixelShader(psm_Tree_cL3, "mesh, shader 'Tree:L3'");
						var program = shader.program = gl.createProgram();
						gl.attachShader(program, vertexShader);
						gl.attachShader(program, pixelShader);
						gl.bindAttribLocation(program, 1, '_1');
				...
				doneGlobal: function(global)
				{
					...
					gl.deleteProgram(global.shaders.m_Tree_cStem1.program);
					gl.deleteProgram(global.shaders.m_Tree_cL3.program);
				},

				render: {
					m_Tree_cStem1_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_Tree_cStem1;
						var transfer = instance.shaders.m_Tree_cStem1.transfer;
						var _a = transfer._a;
					...
					m_Tree_cL3_5: function(renderJob)
					{
						var instance = renderJob.instance;
						var global = instance.global;
						var shader = global.shaders.m_Tree_cL3;
						var transfer = instance.shaders.m_Tree_cL3.transfer;
						var _a = transfer._a;
						var matrix = renderJob.data;
						var flip;
						var _c = instance.fu0_28;
				...
				draw: {
					a: function(instance, shader)
					{
						...
						gl.drawElements(gl.TRIANGLES, 116358, gl.UNSIGNED_SHORT, 0);
					},
					b: function(instance, shader)
					{
						...
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 1867668);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 1867656);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 39654, gl.UNSIGNED_SHORT, 546606);
					},
					c: function(instance, shader)
					{
						var global = instance.global;
						gl.bindBuffer(gl.ARRAY_BUFFER, global.shaderInputBuffer0);
						gl.vertexAttribPointer(1, 3, gl.FLOAT, false, 24, 885300);
						gl.vertexAttribPointer(0, 3, gl.FLOAT, false, 24, 885288);
						gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, global.indexBuffer0);
						gl.drawElements(gl.TRIANGLES, 156945, gl.UNSIGNED_SHORT, 232716);
					}
				},
				...
				createInstance: function(global, renderer)
				{
					...
					instance.update = function()
					{
						...
						ftransforms[31] = 1.0;
						itransforms[1] = 1;
						fboundingBoxes[0] = 0.603849;
						fboundingBoxes[1] = 72.5238;
						fboundingBoxes[2] = -0.9324;
						fboundingBoxes[3] = 57.9478;
						fboundingBoxes[4] = 73.0774;
						fboundingBoxes[5] = 57.8839;
						fboundingBoxes[6] = 0.00674819;
						fboundingBoxes[7] = 88.9068;
						fboundingBoxes[8] = -0.9184;
						fboundingBoxes[9] = 58.6604;
						fboundingBoxes[10] = 53.3616;
						fboundingBoxes[11] = 57.6671;
					};
					...
					instance.render = function(viewMatrix, projectionMatrix, layerName, renderQueues)
					{
						...
						// mesh, shader 'Tree:Stem1'
						var s_ = instance.shaders.m_Tree_cStem1;
						...
						s_.renderJobs = null;
						// mesh, shader 'Tree:L3'
						var s_ = instance.shaders.m_Tree_cL3;
						var uniform = s_.uniform;
						var _b = uniform._b;
						var _a = s_.transfer._a;
						_a[0] = viewMatrix[0];
						...
						if (itransforms[0])
						{
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_Tree_cStem1;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_Tree_cStem1_5;
							...
							if (jobIt != jobEnd)
							{
								var shader = instance.shaders.m_Tree_cL3;
								var renderJob;
								renderJob = jobEnd;
								jobEnd = jobEnd.p;
								renderJob.next = shader.renderJobs;
								shader.renderJobs = renderJob;
								renderJob.render = global.render.m_Tree_cL3_5;
								var data = renderJob.data;
								data[0] = ftransforms[0];
								data[1] = ftransforms[1];
								data[2] = ftransforms[2];
						...
						var uniform = instance.shaders.m_Tree_cStem1.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_Tree_cStem1.renderJobs;
						while (current)
						{
						...
						var shader = global.shaders.m_Tree_cL3;
						gl.useProgram(shader.program);
						var uniform = instance.shaders.m_Tree_cL3.uniform;
						gl.uniform4fv(shader._b, uniform._b);
						gl.enable(gl.CULL_FACE);
						var current = instance.shaders.m_Tree_cL3.renderJobs;
						while (current)
						{
							current.render(current);
							current = current.next;
